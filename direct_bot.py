    ...
    def auth():
        ...
        for msg in range(len(week)):
            ...
            if msg == 0 and message.text == days[0]:
                ...
                #
                # skip_index, skip_flag = 0, False
                #
                # double_cabinets_index = 0
                # double_cabinets_flag = False
                #
                # double_teachers_index = 0
                # double_teachers_flag = False
                #
                # for item_index in range(len(week[msg])):
                #
                #     #назва предмету
                #     if (item_index - double_cabinets_index - double_teachers_index) in [0, 3, 6, 9, 12, 15]:
                #         print(week[msg][item_index])
                #         # ставимо емоджі цифру
                #         final_msg += f'<b>{numbers_list[emoji_index]}{week[msg][item_index]}</b>\n'
                #         emoji_index += 1
                #
                #     #ім'я викладача
                #     elif (item_index - double_cabinets_index - double_teachers_index) in [1, 4, 7, 10, 13, 16]:
                #         # print(week[msg][item_index])
                #
                #         if double_teachers_flag:
                #             double_teachers_index += 1
                #             double_cabinets_flag = False
                #
                #         skip_name_index = str(week[msg][item_index]).find(' ')
                #         final_msg += f'{emoji_list[0]} {week[msg][item_index][skip_name_index+1:]}\n'
                #         double_teachers_flag = True
                #
                #     #вид заняття
                #     elif str(week[msg][item_index])[0].isdigit():
                #
                #         if double_cabinets_flag:
                #             double_cabinets_index += 1
                #             double_cabinets_flag = False
                #             continue
                #
                #         skip_cabinet_index = str(week[msg][item_index]).find(' ')
                #         final_msg += f'{emoji_list[1]} {week[msg][item_index][skip_cabinet_index + 1:]}\n'
                #         # print(week[msg][item_index])
                #
                #         double_cabinets_flag = True
                #         # print(emoji_list[1])
            ...
            if msg == 1 and message.text == days[1]:
                skip_index, skip_flag = 0, False

                for item in week[msg]:
                    print(item)
                    print()
                    if len(item) == 0:
                        final_msg += '-\n\n'
                    ...
                    if item[0].isdigit() and not skip_flag:
                        final_msg += str(item) + '\n\n'
                        skip_flag = True
                        continue
                    final_msg += '\n' + str(item) + '\n\n'
